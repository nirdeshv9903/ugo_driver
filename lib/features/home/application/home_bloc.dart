...
import 'package:appzeto_taxi_driver/common/tobitmap.dart';
...
import 'package:appzeto_taxi_driver/common/app_audios.dart';
import 'package:appzeto_taxi_driver/common/app_constants.dart';
import 'package:appzeto_taxi_driver/common/geohasher.dart';
...
import 'package:appzeto_taxi_driver/db/app_database.dart';
import 'package:appzeto_taxi_driver/features/home/application/usecase/ride_usecases.dart';
...
import 'package:appzeto_taxi_driver/common/debouncer.dart';
...
class HomeBloc extends Bloc<HomeEvent, HomeState> {
  ...
  HomeBloc() : super(HomeInitialState()) {
    ...
    on<ShowZoneNavigationEvent>((event, emit) => emit(ShowZoneNavigationState(zoneName:event.zoneName,zoneLatLng:event.zoneLatLng,endTimestamp:event.endTimestamp)));
  ...
  Future<void> captureImage(
    ...
    try {
      ...
      printWrapped("Error capturing image: $e");
      emit(ImageCaptureFailureState());
    }
  }
...
//  Locations
    ...
    data.fold(
      ...
      (success) {
        ...
        if (success.isOnline == true) {
          ...
          showPeakZones = false;
          polygons.removeWhere((element) => element.polygonId.value.contains('peakzone_'));
          polyline.removeWhere((element) => element.polylineId.value.contains('peakzone_'));
          if (Platform.isAndroid) {
            if (status) {
              WakelockPlus.disable();
            }
    ...
    ownersDriver = FirebaseDatabase.instance
      ...
      if (data.snapshot.exists) {
        ...
        datas.forEach((k, v) {
          ...
          }else if((choosenCarMenu == 2 && v['is_active'] == 1&&(v['is_available'] == true||v['is_available'] == false))){
                markers.removeWhere((element) =>
                    element.markerId.toString().contains('owner_${v['id']}'));
          } else if((choosenCarMenu == 1 && (v['is_active'] == 1 || v['is_active'] == 0) &&v['is_available'] == false)){
                markers.removeWhere((element) =>
                    element.markerId.toString().contains('owner_${v['id']}'));
          } else if((choosenCarMenu == 3 && (v['is_active'] == 1 || v['is_active'] == 0) && (v['is_available'] == true||v['is_available'] == false))){
                markers.removeWhere((element) =>
                    element.markerId.toString().contains('owner_${v['id']}'));
          }
        });
  ...
  FutureOr<void> stopVerifyOtp(
    ...
    final data = await serviceLocator<RideUsecases>()
        .stopOtpVerify(stopId: event.stopId, otp: event.otp);
    data.fold(
      (error) {
  ...
  Future<void> loadPeakZones(GetPeakZoneEvent event, Emitter<HomeState> emit) async {
    zoneStreamRemove?.cancel();
    final zones = await fetchPeakZones();
    zoneStreamRemove = FirebaseDatabase.instance.ref("peak-zones").onChildRemoved.listen((e) {
      showDemandArea = false;
      polygons.removeWhere((element) => element.polygonId.value.contains('peakzone_${e.snapshot.key}'));
      polyline.removeWhere((element) => element.polylineId.value.contains('peakzone_${e.snapshot.key}'));
      polyline.removeWhere((element) => element.polylineId.value.contains('peakzone_route_${e.snapshot.key}'));
    ...
    for (var zone in zones) {
      ...
      if (zoneActive =='1') {
        ...
        polygons.removeWhere((element) => element.polygonId.value.contains('peakzone_$zoneId'));
        polyline.removeWhere((element) => element.polylineId.value.contains('peakzone_$zoneId'));
        polyline.removeWhere((element) => element.polylineId.value.contains('peakzone_route_$zoneId'));
      }
    }
  ...
  Future<void> onZoneTapped(List<LatLng> zonePoints,String zoneId,String zoneName,int endTimestamp) async {
    ...
    polyline = {
      Polyline(
        ...
        patterns: [PatternItem.gap(3),PatternItem.dash(15), PatternItem.gap(3)],
      )
    };
   showDemandArea = true;
   add(UpdateEvent()); 
   add(ShowZoneNavigationEvent(zoneName: zoneName,zoneLatLng:target,endTimestamp:endTimestamp));
  }
  ...
  List<LatLng> generateArcPoints(LatLng start, LatLng end, {int count = 60}) {
    ...
    double controlLat = start.latitude + deltaLat / 2 + 0.017; // adjust height for arc
    double controlLng = start.longitude + deltaLng / 2 - 0.00003; // offset sideways
