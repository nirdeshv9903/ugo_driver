...
import 'package:appzeto_taxi_driver/features/driverprofile/domain/models/service_location_model.dart';
...
import 'package:appzeto_taxi_driver/features/account/application/usecase/acc_usecases.dart';
...
class AccBloc extends Bloc<AccEvent, AccState> {
  ...
  Future<void> getDirection(AccEvent event, Emitter<AccState> emit) async {
    ...
    Future<void> showLoader(AccDataLoaderShowEvent event, Emitter<AccState> emit)async{
    if(event.showLoader){
    ...
    if (event.isFromDate) {
      DateTime? picker = await showDatePicker(
        ...
        initialDate: fromDateText.text.isEmpty
            ? DateTime.now()
            : DateTime.parse(fromDateText.text),
        firstDate: DateTime(2023),
        lastDate: toDateText.text.isEmpty
            ? DateTime.now()
            : DateTime.parse(toDateText.text),
      );
